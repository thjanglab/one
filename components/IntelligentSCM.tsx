...
// --- MOCK DATA ---
const EXPERTS = [
    { id: 1, name: '김민수 (Kim Min-su)', role: '사출 성형 전문가', ex: 'Ex-Samsung (20년)', skill: '불량 분석, 수율 최적화', status: 'Available', score: 98, cost: '$150/hr' },
    { id: 2, name: '이지영 (Lee Ji-young)', role: '회로 설계 마스터', ex: 'Ex-LG (15년)', skill: 'PCB 레이아웃, 신호 무결성', status: 'Busy', score: 95, cost: '$200/hr' },
    { id: 3, name: '박정윤 (Park Jung-yoon)', role: '자동화 엔지니어', ex: 'Ex-Hyundai (18년)', skill: 'PLC, 로봇 제어', status: 'Available', score: 92, cost: '$180/hr' },
];
...
const BOM_QUALITY = [
    { 
        ...
        supplier: 'S-Tech', 
        risk: 'Low', 
        reason: '12개월 연속 고수율 유지',
        details: '코일 권선 공정 자동화로 불량률 0.02% 달성. 진동/소음 테스트 전수 검사 통과.',
        action: '현행 유지 및 장기 공급 계약 검토' 
    },
    { 
        name: 'Gearbox', 
        rate: 95.2, 
        ...
        risk: 'Medium', 
        reason: '간헐적 공차 이탈 발생', 
        details: '3번 라인 기어 호빙 머신 정밀도 저하 감지됨. 치수 오차가 허용 범위를 상회하는 빈도 증가.',
        action: '3번 라인 설비 오버홀 및 전수 치수 검사 실시 요청'
    },
    { 
        name: 'Controller', 
        rate: 98.5, 
        supplier: 'ChipSol', 
        risk: 'Low', 
        reason: 'ISO 26262 인증 공정', 
        details: 'PCB 솔더링 공정 AI 비전 검사 도입으로 신뢰성 향상. 열충격 테스트 내구도 우수.',
        action: '표준 부품 지정 및 안전 재고 확대'
    },
    { 
        name: 'Casing', 
        rate: 92.1, 
        supplier: 'MoldPlus', 
        risk: 'High', 
        reason: '냉각 사이클 불안정 감지됨', 
        details: '사출 성형 시 금형 냉각수 온도 편차로 인한 치수 변형 발생. 원재료 수분 함유량 과다 의심.',
        action: '공급사 현장 감사(Audit) 및 원재료 건조 공정 개선 명령'
    },
];

// Diverse Nodes for Digital Twin (Tier 1-4)
const ECOSYSTEM_NODES = [
    { id: 1, name: '현대모비스 (Tier 1)', grade: 'S', score: 98, status: 'Optimal', type: 'Module' },
    { id: 2, name: 'HL만도 (Tier 2)', grade: 'A', score: 92, status: 'Good', type: 'Chassis' },
    { id: 3, name: '포스코 (Tier 3)', grade: 'S', score: 96, status: 'Optimal', type: 'Raw Material' },
    { id: 4, name: 'LG화학 (Tier 4)', grade: 'A', score: 94, status: 'Good', type: 'Chemical' },
    { id: 5, name: '한온시스템 (Tier 2)', grade: 'B', score: 88, status: 'Warning', type: 'Thermal' },
    { id: 6, name: '삼성SDI (Tier 1)', grade: 'S', score: 99, status: 'Optimal', type: 'Battery' },
];
...
const INDUSTRIES = [
    { 
        id: 'AUTO', 
        label: '자동차 (Automotive)', 
        icon: Car, 
        color: 'bg-blue-100 text-blue-600', 
        products: ['EV 배터리 모듈', '구동 모터 어셈블리', '샤시 프레임', '인포테인먼트 시스템'] 
    },
    { 
        id: 'ELEC', 
        label: '전자 (Electronics)', 
        icon: Cpu, 
        color: 'bg-purple-100 text-purple-600', 
        products: ['PCB 메인보드', 'OLED 디스플레이 패널', '카메라 모듈', '전력 반도체'] 
    },
    { 
        id: 'SHIP', 
        label: '조선 (Shipbuilding)', 
        icon: Anchor, 
        color: 'bg-cyan-100 text-cyan-600', 
        products: ['LNG 선박 엔진', '프로펠러 샤프트', '선박용 후판', '항해 통신 장비'] 
    },
];

// --- EVALUATION TARGETS ---
const EVAL_TARGETS = [
    { id: 'S1', name: '미래 테크 (Tier 2)', industry: 'Electronics' },
    { id: 'S2', name: '강철 정밀 (Tier 3)', industry: 'Automotive' },
    { id: 'S3', name: '오션 파츠 (Tier 1)', industry: 'Shipbuilding' },
];
...
const INVENTORY_OPTIMIZATION_DATA = [
    { name: 'Elec. Module', current: 1500, optimal: 1000, status: 'Excess', action: 'Reduce Order' },
    { name: 'Housing', current: 200, optimal: 500, status: 'Shortage', action: 'Urgent Order' },
    { name: 'Connectors', current: 5000, optimal: 4800, status: 'Optimal', action: 'Maintain' },
    { name: 'Display', current: 800, optimal: 800, status: 'Optimal', action: 'Maintain' },
];

// MOCK SUPPLIERS FOR RECOMMENDATION
const RECOMMENDED_SUPPLIERS = [
    { id: 1, name: 'Alpha Tech', score: 98, tag: 'Best Quality', match: '99%', risk: 'Low', location: 'Korea', price: '$$$' },
    { id: 2, name: 'Beta Mfg', score: 94, tag: 'Cost Effective', match: '92%', risk: 'Low', location: 'Vietnam', price: '$' },
    { id: 3, name: 'Gamma Ind', score: 88, tag: 'Fast Delivery', match: '85%', risk: 'Medium', location: 'China', price: '$$' },
];
...
const IntelligentSCM: React.FC = () => {
    ...
    const [chatHistory, setChatHistory] = useState<{role: 'user' | 'bot', content: string, action?: any, targetCompany?: string}[]>([
        { role: 'bot', content: '안녕하세요! 지능형 데이터 카탈로그 에이전트입니다. 무엇을 도와드릴까요?' }
    ]);
    ...
    const [agentChatLog, setAgentChatLog] = useState<{sender: string, msg: string}[]>([
        { sender: 'System', msg: 'AI 에이전트 협상 프로토콜 대기 중...' }
    ]);
    ...
    const handleChatSubmit = (query?: string) => {
        ...
        setTimeout(() => {
            let responseMsg = '';
            // Pick a random company for visualization
            const randomCompany = ECOSYSTEM_NODES[Math.floor(Math.random() * ECOSYSTEM_NODES.length)];
            
            // Default Action for ALL responses to show a report
            let actionData: { type: string; label: string; query: string } = { 
                type: 'CHAT_RESULT', 
                label: 'AI 분석 리포트 보기',
                query: text 
            };

            if (text.includes('불량') || text.includes('품질')) {
                responseMsg = `지난달 불량률 분석 결과, "${randomCompany.name}"의 부품에서 간헐적 공차 이탈이 발견되었습니다. 상세 품질 분석 리포트를 생성했습니다.`;
                actionData.query = '품질 결함 분석 (Defect Analysis)';
            } else if (text.includes('계약') || text.includes('갱신')) {
                responseMsg = `현재 "${randomCompany.name}"와의 공급 계약 갱신 시점이 도래했습니다. 표준 계약 초안 및 공급사 평가 보고서를 준비했습니다.`;
                actionData.query = '계약 갱신 검토 (Contract Review)';
            } else {
                responseMsg = `"${text}"에 대한 데이터셋 14건을 찾았으며, "${randomCompany.name}" 관련 공급망 인사이트를 분석했습니다.`;
            }
    ...
    const handleChatAction = (action: any) => {
        if (action.type === 'CHAT_RESULT') {
            setReportData({
                title: 'AI 인텔리전스 분석 리포트',
                targetCompany: '대상 업체: ' + (chatHistory[chatHistory.length - 1].targetCompany || 'MoldPlus Inc.'),
                topic: action.query,
                summary: 'AI가 1.2TB의 공급망 데이터를 분석했습니다. 귀하의 질의와 관련된 주요 패턴과 리스크 요인을 식별했습니다.',
                insights: [
                    { label: '데이터 신뢰도', value: '98%' },
                    { label: '시장 추세', value: '상승세' },
                    { label: '리스크 점수', value: '낮음 (12/100)' }
                ],
                recommendation: 'Q3 재고 확보를 위한 선제적 발주 및 공급망 다변화를 권장합니다.',
                chartData: [
    ...
    const runContractFlow = () => {
        ...
        setTimeout(() => {
            setReportData({
                ...
                reasoning: 'AI 분석 결과: ESG 점수(92점) 및 납기 준수율(99.8%) 최우수 업체로 선정됨. 공급 리스크가 가장 낮음.',
                supplierScore: 92
            });
    ...
    const runOrderSync = () => {
        ...
        setTimeout(() => {
            ...
            setReportData({
                ...
                industry: selectedIndustry ? INDUSTRIES.find(i => i.id === selectedIndustry)?.label : '일반',
                targetProduct: selectedProduct || '모듈',
                items: [
                    { tier: 'Tier 1', supplier: '현대모비스', part: '모듈 A', status: '수신확인', sn: 'SN-A-001', eta: 'D+2' },
                    { tier: 'Tier 2', supplier: '현대위아', part: '기어 세트', status: '수신확인', sn: 'SN-B-102', eta: 'D+1' },
                    { tier: 'Tier 3', supplier: 'POSCO', part: '강철 코일', status: '수신확인', sn: 'SN-C-882', eta: '준비완료' }
                ],
                totalQty: 5000,
                syncTime: '1.2s',
                optimization: 'AI 최적화 경로를 통해 전체 리드타임 14% 단축 예상.'
            });
    ...
    const handleExpertConnect = (expert: any) => {
        setReportData({
            ...
            rate: expert.cost,
            reasoning: `최근 검색하신 "사출 성형 불량" 키워드와 98% 일치합니다. ${expert.name}님은 ${expert.ex} 분야에서 유사 문제 해결 경험이 풍부합니다.`,
            contractTerms: '표준 자문 계약 (NDA 포함)',
            radar: [
                { subject: '기술 적합성', A: expert.score, fullMark: 100 },
                { subject: '가용성', A: expert.status === 'Available' ? 100 : 50, fullMark: 100 },
                { subject: '비용 효율', A: 85, fullMark: 100 },
                { subject: '경력', A: 95, fullMark: 100 },
                { subject: '평판', A: 90, fullMark: 100 },
            ]
        });
    ...
    const handleBOMDetail = (item: any) => {
        setReportData({
            ...
            metric: `합격률: ${item.rate}%`,
            rootCause: item.details || item.reason, // Use detailed reason
            riskLevel: item.risk,
            recommendation: item.action || (item.risk === 'High' ? '공급사 현장 감사 및 시정 조치(CAPA) 요청' : '현행 모니터링 유지'),
            trendData: [
                { name: '1주차', value: item.rate - 2 }, { name: '2주차', value: item.rate - 1 }, 
                { name: '3주차', value: item.rate + 1 }, { name: '4주차', value: item.rate }
            ],
            contractImpact: '제 7.2조 (품질 보증) 위반 소지 있음. 페널티 검토 필요.'
        });
    ...
    const handleBomFullReport = () => {
        ...
        const data = {
            ...
            treeData: [
                { name: 'Powertrain', size: 1200, color: '#3b82f6' },
                { name: 'Chassis', size: 800, color: '#10b981' },
                { name: 'Electronics', size: 600, color: '#8b5cf6' },
                { name: 'Body', size: 400, color: '#f59e0b' },
                { name: 'Interior', size: 300, color: '#64748b' }
            ],
            topRisks: [
                { id: 'P-102', name: 'Casing', reason: 'Dimension Error', supplier: 'MoldPlus', severity: 'High' },
                { id: 'E-221', name: 'PCB Board', reason: 'Solder Defect', supplier: 'ChipSol', severity: 'Medium' },
                { id: 'G-303', name: 'Gearbox', reason: 'Vibration', supplier: 'K-Gear', severity: 'Medium' }
            ]
        };
        setReportData(data);
    ...
    const handleInventoryClick = (item: any) => {
        ...
        setReportData({
            ...
            action: item.action,
            leadTime: '3 Days',
            consumptionRate: '35 units/day',
            nextOrder: '2024-05-28 (Recommended)',
    ...
    const simulateDemandSpike = () => {
        ...
        setReportData({
            ...
            aiInsight: 'Warning: Simulated +20% demand spike leads to stockout on D+12. Immediate replenishment required.'
        });
    };
    ...
    const handleForecastClick = () => {
        setReportData({
            title: "Demand Forecasting AI Analysis",
            metrics: {
                mape: { before: "15%", after: "2.1%" },
                stockout: { before: "4 events/yr", after: "0 events/yr" },
    ...
    const handleSupplierRecommendation = () => {
        setReportData({
            title: "AI Optimal Supplier Sourcing",
            candidates: 1420,
            qualified: 24,
            shortlist: 3,
            criteria: ["Cost Efficiency", "Quality Certs", "ESG Score", "Delivery Speed"],
            suppliers: RECOMMENDED_SUPPLIERS,
            comparisonData: [
                { attribute: 'Quality', alpha: 98, beta: 90, gamma: 88, full: 100 },
                { attribute: 'Cost', alpha: 80, beta: 98, gamma: 90, full: 100 },
                { attribute: 'Delivery', alpha: 95, beta: 85, gamma: 99, full: 100 },
                { attribute: 'ESG', alpha: 92, beta: 88, gamma: 85, full: 100 },
                { attribute: 'Risk', alpha: 95, beta: 92, gamma: 80, full: 100 },
            ]
        });
    ...
    const runAgentEval = () => {
        ...
        setAgentChatLog([{ sender: 'System', msg: `${selectedSupplier || 'Target'} 대상 AI 평가 시작...` }]); 
        const sequence = [
            { sender: 'Buyer Bot', msg: '보유 역량 증빙 자료 요청 중...' },
            { sender: 'Supplier Bot', msg: 'ISO 9001 인증서 제출 완료.' },
            { sender: 'Supplier Bot', msg: '1분기 생산 수율 로그 제출...' },
            { sender: 'Buyer Bot', msg: '블록체인을 통한 문서 진위 검증 중...' },
            { sender: 'Buyer Bot', msg: '분석 완료. 신용 등급 및 기술 점수 산출됨.' }
        ];
        let i = 0;
        ...
        evalIntervalRef.current = setInterval(() => {
            if (i >= sequence.length) {
                ...
                setReportData({
                    ...
                    grade: 'A-',
                    score: 92,
                    strengths: ['가격 경쟁력 우수', '친환경 공정 인증'],
                    weaknesses: ['납기 일관성 다소 부족'],
                    radar: [
                        { subject: '품질', A: 90, fullMark: 100 },
                        { subject: '비용', A: 95, fullMark: 100 },
                        { subject: '납기', A: 80, fullMark: 100 },
                        { subject: 'ESG', A: 98, fullMark: 100 },
                        { subject: '기술력', A: 85, fullMark: 100 },
                    ]
                });
    ...
    const runEOSimulation = () => {
        setReportData({
            eoId: 'EO-24-05-002',
            change: '재질 변경 (Steel -> Aluminum)',
            stockImpact: [
                { date: '5월 10일', stock: 4500, status: '안전' },
                { date: '5월 11일', stock: 3200, status: '안전' },
                { date: '5월 12일', stock: 100, status: '위험' }, // Run-out
                { date: '5월 13일', stock: 0, status: '소진' },
            ],
            optimalDate: '5월 13일, 08:00 AM',
            costSaving: '$12,500 (폐기 비용 절감)',
            // Updated with Supply Chain Map companies - Explicitly matching Recommended Suppliers
            suppliers: RECOMMENDED_SUPPLIERS.map(s => ({ ...s, role: 'Alternative Supplier' })),
            // AI Comparison Data
            processMap: {
                manual: ["Engineering Change Notice (Email)", "Manual Stock Check", "Supplier Call", "PO Update", "Risk: High"],
                ai: ["Auto-Detection", "Real-time Stock Sync", "Smart Contract Update", "Auto-Balancing", "Risk: None"]
            },
            impactMetrics: {
                time: { before: "5 Days", after: "2 Hours" },
                cost: { before: "$15,000", after: "$2,500" },
                risk: { before: "High (Line Stop)", after: "Low (Seamless)" }
            }
        });
                    ...
                    <div className="p-8">
                        <div className="mb-6 text-center">
                            <h2 className="text-2xl font-bold text-slate-900 mb-2">
                                {configMode === 'CONTRACT' ? '계약 조건 설정' : configMode === 'ORDER' ? '동시 발주 설정' : 'AI 평가 대상 설정'}
                            </h2>
                            <p className="text-slate-500 text-sm">
                                프로세스를 시작하기 위해 대상을 선택해주세요.
                            </p>
                        </div>
                                ...
                                {INDUSTRIES.map(ind => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">선택하여 세부 품목/업체 보기</p>
                                        </div>
                        ...
                        {configStep === 2 && (
                            <div className="space-y-4 animate-fadeIn">
                                <div className="flex items-center gap-2 mb-2">
                                    <button onClick={() => setConfigStep(1)} className="text-sm text-slate-400 hover:text-slate-600">&larr; 뒤로</button>
                                    <span className="text-sm font-bold text-slate-900">/ {isEval ? '평가 대상 업체 선택' : '대상 품목 선택'}</span>
                                </div>
                        ...
                        {configStep === 3 && (
                            <div className="text-center animate-fadeIn">
                                <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6 relative overflow-hidden">
                                    ...
                                    <h4 className="text-sm font-bold text-slate-500 uppercase mb-4">설정 요약</h4>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <span className="text-slate-500">{isEval ? '대상 업체' : '대상 품목'}</span>
                                    ...
                                    <div className="mt-6 flex justify-center items-center gap-4 text-xs text-slate-400">
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            <span className="text-blue-600 font-bold">AI 에이전트</span>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-scaleUp flex flex-col max-h-[90vh] relative">
                    ...
                    <div className="p-6 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                        <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                            ...
                            {activeReport === 'CONTRACT' && '스마트 계약 체결 리포트'}
                            {activeReport === 'ORDER' && '동시 발주(Sync) 리포트'}
                            {activeReport === 'QUALITY_ANALYSIS' && '품질 정밀 분석 리포트'}
                            {activeReport === 'SUPPLIER_EVAL' && 'AI 공급사 평가 리포트'}
                            {activeReport === 'EO_IMPACT' && '설계 변경(EO) 영향도 분석'}
                            {activeReport === 'CHAT_RESULT' && 'AI 인텔리전스 분석'}
                            {activeReport === 'EXPERT_CONNECT' && '전문가 매칭 계약서'}
                            {activeReport === 'INVENTORY_REPORT' && '재고 최적화 및 수요 예측'}
                            {activeReport === 'BOM_FULL_REPORT' && 'BOM 통합 품질 대시보드'}
                            {activeReport === 'FORECAST_REPORT' && 'AI 수요 예측 비교 분석'}
                            {activeReport === 'SUPPLIER_RECOMMENDATION' && 'AI 최적 협력사 추천 리포트'}
                        </h3>
                        ...
                        {activeReport === 'FORECAST_REPORT' && (
                            <div className="space-y-6">
                                ...
                                <div className="bg-gradient-to-r from-pink-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
                                    <h2 className="text-2xl font-bold mb-2">AI Demand Forecasting Analysis</h2>
                                    <p className="opacity-90 text-sm">Comparing Legacy (Manual) vs. AI-Driven Forecasting Performance</p>
                                    
                                    <div className="grid grid-cols-3 gap-4 mt-6">
                                        <div className="bg-white/10 backdrop-blur rounded-lg p-3 border border-white/20">
                                            <span className="block text-xs uppercase opacity-70 font-bold">Accuracy Improvement</span>
                                            <span className="text-2xl font-bold flex items-center gap-2">
                                                +12.9% <ArrowRight className="w-4 h-4" />
                                            </span>
                                        </div>
                                        <div className="bg-white/10 backdrop-blur rounded-lg p-3 border border-white/20">
                                            <span className="block text-xs uppercase opacity-70 font-bold">Inventory Cost</span>
                                            <span className="text-2xl font-bold flex items-center gap-2">
                                                -33% <TrendingDown className="w-4 h-4" />
                                            </span>
                                        </div>
                                        <div className="bg-white/10 backdrop-blur rounded-lg p-3 border border-white/20">
                                            <span className="block text-xs uppercase opacity-70 font-bold">Stockout Events</span>
                                            <span className="text-2xl font-bold flex items-center gap-2">
                                                0 <span className="text-sm font-normal opacity-70">/ Year</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                    <h4 className="font-bold text-slate-900 mb-6 flex items-center gap-2">
                                        ...
                                        Legacy vs AI Forecast Accuracy
                                    </h4>
                                    <div className="h-72 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <ComposedChart data={reportData.data}>
                                                ...
                                                <Tooltip />
                                                <Legend />
                                                <Area type="monotone" dataKey="actual" name="Actual Sales" fill="#bfdbfe" stroke="#3b82f6" fillOpacity={0.2} />
                                                <Line type="monotone" dataKey="legacy" name="Legacy Forecast" stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" />
                                                <Line type="monotone" dataKey="forecast" name="AI Forecast" stroke="#db2777" strokeWidth={3} />
                                            </ComposedChart>
                                        </ResponsiveContainer>
                                    </div>
                                    <div className="mt-4 p-3 bg-pink-50 text-pink-800 text-xs rounded border border-pink-100 flex items-start gap-2">
                                        ...
                                        <p>
                                            <strong>Analysis:</strong> The legacy model consistently over-forecasted in Q1 and under-forecasted in Q2 due to seasonality lag. The AI model adapted to the trend change within 14 days, preventing a potential stockout in May.
                                        </p>
                                    </div>
                                </div>
                                        ...
                                        <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="text-center">
                                                    ...
                                                    <div className="text-[10px] text-emerald-500 font-bold uppercase">After AI</div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        {activeReport === 'SUPPLIER_RECOMMENDATION' && (
                            <div className="space-y-8 animate-fadeIn">
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                                    <h4 className="text-lg font-bold text-slate-900 mb-6 text-center">AI Strategic Sourcing Funnel</h4>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {reportData.suppliers.map((sup: any, i: number) => (
                                            <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                                                {i === 0 && <div className="absolute top-0 right-0 bg-yellow-400 text-yellow-900 text-[10px] font-bold px-2 py-0.5 rounded-bl">#1 CHOICE</div>}
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <span className="px-2 py-1 bg-slate-50 text-slate-600 text-xs font-bold rounded border border-slate-200">Risk: {sup.risk}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                                        <span className="text-xs font-bold text-slate-500 uppercase mb-2">Current Stock</span>
                                        ...
                                        <span className="text-xs text-slate-400 mt-1">Units</span>
                                    </div>
                                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                                        <span className="text-xs font-bold text-slate-500 uppercase mb-2">Optimal Level</span>
                                    ...
                                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                                        <span className="text-xs font-bold text-slate-500 uppercase mb-2">Next Order</span>
                                        <span className="text-xl font-bold text-emerald-600 mt-2">
                                            {reportData.nextOrder.split('(')[0]}
                                        </span>
                                        <span className="text-xs text-slate-400 mt-1">AI Recommendation</span>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                    <div className="flex justify-between items-center mb-6">
                                        <h4 className="font-bold text-slate-900 flex items-center gap-2">
                                            ...
                                            AI Inventory Simulation (30 Days)
                                        </h4>
                                        <button 
                                        ...
                                        >
                                            <AlertTriangle className="w-3 h-3" /> Simulate Demand Spike (+20%)
                                        </button>
                                    </div>
                                    <div className="h-64 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <ComposedChart data={reportData.trendData || invSimulationData}>
                                                ...
                                                <Tooltip />
                                                <Legend />
                                                <Area type="monotone" dataKey="stock" name="Projected Stock" fill="#bfdbfe" stroke="#3b82f6" />
                                                <Line type="monotone" dataKey="optimal" name="Optimal Level" stroke="#10b981" strokeDasharray="5 5" dot={false} />
                                                <Line type="monotone" dataKey="safety" name="Safety Stock" stroke="#ef4444" strokeDasharray="3 3" dot={false} />
                                            </ComposedChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                                        <h4 className="font-bold text-slate-900 mb-3">Suggested Actions</h4>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Total Items</div>
                                        <div className="text-2xl font-bold text-slate-900">{reportData.totalItems}</div>
                                    </div>
                                    <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-center">
                                        <div className="text-xs text-emerald-600 uppercase font-bold mb-1">Avg. Yield</div>
                                        <div className="text-2xl font-bold text-emerald-800">{reportData.avgYield}</div>
                                    </div>
                                    <div className="bg-red-50 p-4 rounded-xl border border-red-100 text-center">
                                        <div className="text-xs text-red-600 uppercase font-bold mb-1">High Risk Items</div>
                                        <div className="text-2xl font-bold text-red-800">{reportData.riskItems}</div>
                                    </div>
                                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-center">
                                        <div className="text-xs text-blue-600 uppercase font-bold mb-1">Suppliers</div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                                        <h4 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                                            ...
                                            Defect Rate Trend (Last 6 Months)
                                        </h4>
                                        <div className="h-64 w-full">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={reportData.defectTrend}>
                                                    ...
                                                    <Bar dataKey="rate" fill="#8b5cf6" radius={[4, 4, 0, 0]} name="Defect %">
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                                    <div className="p-4 bg-slate-50 border-b border-slate-100 font-bold text-slate-900 flex items-center gap-2">
                                        ...
                                        Priority Risk Items (Action Required)
                                    </div>
                        ...
                        {activeReport === 'CHAT_RESULT' && (
                            <div className="space-y-6">
                                <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100 flex gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-purple-900 text-lg mb-2">"{reportData.topic}" 분석 결과</h4>
                                ...
                                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-xl">
                                    <h5 className="text-sm font-bold text-blue-800 mb-1">AI 추천 사항</h5>
                        ...
                        {activeReport === 'EXPERT_CONNECT' && (
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-6 items-center">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex gap-2 mt-4">
                                            ...
                                            <span className="bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-full text-xs font-bold text-emerald-700">Match Score: {reportData.matchScore}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-1"><Lightbulb className="w-3 h-3" /> Selection Reasoning</h4>
                                            <p className="text-sm text-slate-700 leading-relaxed">{reportData.reasoning}</p>
                                        </div>
                                        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                                            <h4 className="text-xs font-bold text-indigo-800 uppercase mb-2">Primary Skills</h4>
                                ...
                                <div className="border-t-2 border-dashed border-slate-300 pt-6 mt-6">
                                    <h4 className="text-center font-serif text-xl font-bold text-slate-800 mb-4">Consulting Service Agreement</h4>
                                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="flex justify-between items-end mt-8">
                                            ...
                                            <div className="text-center">
                                                ...
                                                <div className="border-t border-slate-300 w-32 pt-1 text-[10px] text-slate-400 uppercase">Expert Signature</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {activeReport === 'CONTRACT' && (
                            <div className="space-y-8">
                                ...
                                <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100 flex items-center gap-4">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-bold text-emerald-900">계약 체결 완료 (Success)</h2>
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex-1 bg-slate-50 rounded-lg p-3 border border-slate-100">
                                            <span className="text-xs text-slate-500 block">AI 평가 점수</span>
                                            <span className="text-xl font-bold text-blue-600">{reportData.supplierScore}/100</span>
                                        </div>
                                        <div className="flex-1 bg-slate-50 rounded-lg p-3 border border-slate-100">
                                            <span className="text-xs text-slate-500 block">리스크 레벨</span>
                                            <span className="text-xl font-bold text-emerald-600">Low (안전)</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="border-4 border-double border-slate-200 p-8 rounded-lg bg-white relative">
                                    ...
                                    <h3 className="text-center font-serif text-2xl font-bold mb-6 text-slate-900 underline decoration-slate-300 decoration-1 underline-offset-4">표준 공급 계약서</h3>
                                    ...
                                    <div className="space-y-4 text-sm text-slate-600 font-serif leading-relaxed">
                                        <p><strong>제 1조 (목적)</strong> 본 계약은 {reportData.product}의 공급에 관한 제반 사항을 규정함을 목적으로 한다.</p>
                                        <p><strong>제 2조 (기간 및 조건)</strong> {reportData.terms}</p>
                                        <div className="bg-yellow-50 p-3 border border-yellow-100 rounded">
                                            <strong>제 3조 (특약 사항)</strong> {reportData.clause}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-between mt-12 pt-8 border-t border-slate-200">
                                        <div className="text-center">
                                            ...
                                            <span className="text-xs text-slate-400 uppercase mt-1 block">구매자 서명</span>
                                        </div>
                                        <div className="text-center">
                                            <div className="font-script text-3xl text-slate-800 mb-2">S-Tech CEO</div>
                                            <div className="h-px w-40 bg-slate-400 mx-auto"></div>
                                            <span className="text-xs text-slate-400 uppercase mt-1 block">공급자 서명</span>
                                        </div>
                                    </div>
                        ...
                        {activeReport === 'ORDER' && (
                            <div className="space-y-6">
                                <div className="flex justify-between items-center bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                                    <div>
                                        <p className="text-sm text-emerald-800 font-bold">동시 발주 총 수량</p>
                                        <p className="text-2xl font-bold text-emerald-600">{reportData.totalQty}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-emerald-700">동기화 지연시간</p>
                                        <p className="font-mono font-bold">{reportData.syncTime}</p>
                                    </div>
                                </div>

                                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                                    <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2"><Zap className="w-4 h-4 text-amber-500" /> AI 최적화 효과</h4>
                                    ...
                                    <div className="flex justify-between text-xs mt-1 text-slate-500">
                                        <span>표준 리드타임</span>
                                        <span className="text-emerald-600 font-bold">단축된 시간</span>
                                    </div>
                                </div>
                                ...
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-slate-100 text-slate-600">
                                        <tr>
                                            ...
                                            <th className="p-3">품목 / S.N</th>
                                            <th className="p-3">예상도착</th>
                        ...
                        {activeReport === 'QUALITY_ANALYSIS' && (
                            <div className="space-y-6">
                                <div className="p-4 bg-red-50 border border-red-100 rounded-xl flex items-start gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-red-800 text-lg">품질 경보: {reportData.target}</h4>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white p-4 rounded-xl border border-slate-200">
                                        <h5 className="font-bold text-slate-800 mb-2 text-sm">근본 원인 (AI 분석)</h5>
                                    ...
                                    <div className="bg-white p-4 rounded-xl border border-slate-200">
                                        <h5 className="font-bold text-slate-800 mb-2 text-sm">계약 위반 사항</h5>
                                ...
                                <div className="bg-white p-4 rounded-xl border border-slate-200 h-48">
                                    <h5 className="font-bold text-slate-800 mb-2 text-sm">트렌드 분석 (최근 4주)</h5>
                                    <ResponsiveContainer width="100%" height="100%">
                                ...
                                <div>
                                    <h5 className="font-bold text-slate-800 mb-2">권장 조치 사항</h5>
                            ...
                            <div className="space-y-6">
                                <div className="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">AI 자동 평가 리포트</p>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-4xl font-bold text-blue-600">{reportData.grade}</div>
                                        <div className="text-xs font-bold text-slate-400">종합 점수: {reportData.score}</div>
                                    </div>
                                </div>
                                ...
                                <div className="h-64 w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={reportData.radar}>
                                            ...
                                            <Radar name="Supplier" dataKey="A" stroke="#2563eb" fill="#3b82f6" fillOpacity={0.6} />
                                            <Legend />
                                        </RadarChart>
                                    </ResponsiveContainer>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                                        <span className="text-xs font-bold text-emerald-700 uppercase block mb-1">강점 (Strengths)</span>
                                    ...
                                    <div className="p-3 bg-red-50 rounded-lg border border-red-100">
                                        <span className="text-xs font-bold text-red-700 uppercase block mb-1">약점 (Weaknesses)</span>
                            ...
                            <div className="space-y-6">
                                <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-orange-700 mt-1">최적 적용 시점: <strong>{reportData.optimalDate}</strong></p>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-sm text-orange-600 font-bold">Risk Level</div>
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 gap-8">
                                        ...
                                        <div className="relative pl-8 border-l-2 border-slate-200 border-dashed">
                                            ...
                                            <h5 className="text-sm font-bold text-slate-500 mb-3">Legacy Manual Process (5 Days)</h5>
                                        ...
                                        <div className="relative pl-8 border-l-2 border-emerald-500">
                                            ...
                                            <h5 className="text-sm font-bold text-emerald-600 mb-3">AI Automated Process (2 Hours)</h5>
                                            <div className="flex gap-2 overflow-x-auto pb-2">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Time to Apply</div>
                                    ...
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Cost Impact</div>
                                        <div className="flex justify-center items-baseline gap-2">
                                    ...
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Risk Reduction</div>
                                ...
                                {reportData.suppliers && (
                                    <div className="mt-4">
                                        <h5 className="font-bold text-slate-800 mb-2 flex items-center gap-2">
                                            ...
                                            Optimal Supplier Recommendation
                                        </h5>
                                        <div className="grid grid-cols-1 gap-3">
                                            {reportData.suppliers.map((sup: any, i: number) => (
                                                <div key={i} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg shadow-sm">
                                                    ...
                                                    <div className="text-right">
                                                        <div className="text-emerald-600 font-bold">{sup.score}점</div>
                    ...
                    <div className="p-6 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
                        ...
                        <button className="px-4 py-2 border border-slate-300 rounded-lg text-slate-600 font-bold hover:bg-white flex items-center gap-2">
                            <Download className="w-4 h-4" /> 저장 (PDF)
                        </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fadeIn">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[500px]">
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
                            ...
                            <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-1 rounded-full font-bold">AI Active</span>
                        </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative group overflow-hidden">
                            ...
                            {contractStep > 0 ? (
                                ...
                                <div className="flex flex-col gap-4">
                                    <div className="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-100">
                                        ...
                                        <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${contractStep >= 1 ? 'opacity-100 scale-105' : 'opacity-50'}`}>
                                            ...
                                            <span className="text-xs font-bold text-slate-600">AI Review</span>
                                        </div>
                                        <div className="h-0.5 flex-1 bg-slate-200 mx-2 relative">
                                        ...
                                        <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${contractStep >= 2 ? 'opacity-100 scale-105' : 'opacity-50'}`}>
                                            ...
                                            <span className="text-xs font-bold text-slate-600">Signed</span>
                                        </div>
                                    </div>
                                    <div className="text-center text-sm font-bold text-blue-600 animate-pulse h-6">
                                        {contractStep === 1 && '초안 생성 중...'}
                                        {contractStep === 2 && 'AI 리스크 검토 중...'}
                                        {contractStep === 3 && '계약 체결 완료 (Contract Finalized)'}
                                    </div>
                                    ...
                                    {contractStep === 3 && (
                                        ...
                                        >
                                            <RefreshCw className="w-3 h-3" /> Start New Contract
                                        </button>
                                    )}
                                </div>
                            ) : (
                                ...
                                <div className="flex flex-col gap-4 relative z-10">
                                    <p className="text-sm text-slate-500">스마트 계약을 자동으로 생성하고 체결합니다.</p>
                                    <button 
                        ...
                        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden group">
                                ...
                                <div className="flex gap-2">
                                    {orderStatus === 'COMPLETED' ? (
                                        ...
                                        >
                                            ...
                                            {orderStatus === 'SYNCING' ? '동기화 중...' : '발주 동기화'}
                                        </button>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="absolute bottom-4 right-6 text-xs text-slate-400 font-mono flex items-center gap-2">
                                ...
                                Tracking Serial: {orderStatus === 'SYNCING' ? 'SYNCING...' : orderStatus === 'COMPLETED' ? 'CONFIRMED' : '#SN-2024-X99'}
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="h-64 w-full mb-4 cursor-pointer" onClick={() => handleBOMDetail(BOM_QUALITY[0])}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={BOM_QUALITY} layout="vertical" margin={{ left: 10 }}>
                                    ...
                                    <Tooltip cursor={{fill: 'transparent'}} />
                                    <Bar dataKey="rate" name="Pass Rate %" radius={[0, 4, 4, 0]}>
                        ...
                        <div className="space-y-2">
                            {BOM_QUALITY.map((item, i) => (
                                ...
                                >
                                    ...
                                    <span className={item.risk === 'High' ? 'text-red-500 font-bold' : 'text-slate-500'}>Risk: {item.risk}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                                ...
                                <div 
                                    ...
                                    onClick={() => {
                                        setReportData({
                                            ...
                                            grade: node.grade,
                                            score: node.score,
                                            strengths: ['Stable Quality', 'Data Transparency'],
                                            weaknesses: ['Cost Structure'],
                                            radar: [
                                                { subject: '품질', A: node.score, fullMark: 100 },
                                                { subject: '비용', A: node.score - 10, fullMark: 100 },
                                                { subject: '납기', A: node.score - 5, fullMark: 100 },
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="relative h-40 flex items-center mb-4">
                            ...
                            <div className="absolute left-[10%] top-1/2 -translate-y-1/2 flex flex-col items-center group cursor-pointer">
                                ...
                                <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">EO Issued</span>
                            </div>
                            ...
                            <div className="absolute left-[40%] top-1/2 -translate-y-1/2 flex flex-col items-center group cursor-pointer">
                                ...
                                <span className="text-[10px] font-bold text-red-500 bg-red-50 px-2 py-1 rounded border border-red-100">Stock Run-out</span>
                            ...
                            <div className="absolute left-[70%] top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group cursor-pointer">
                                ...
                                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-100">New Ver. Start</span>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fadeIn">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm cursor-pointer hover:border-pink-300 transition-all" onClick={handleForecastClick}>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button className="text-xs text-pink-600 font-bold hover:underline flex items-center gap-1">
                                View AI Impact Report <ArrowRight className="w-3 h-3" />
                            </button>
                        </div>
                        <div className="h-64 w-full mb-4">
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={DEMAND_FORECAST_DATA}>
                                    ...
                                    <Tooltip />
                                    <Legend />
                                    <Area type="monotone" dataKey="actual" name="Actual Sales" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} />
                                    <Line type="monotone" dataKey="legacy" name="Legacy Forecast" stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" />
                                    <Line type="monotone" dataKey="forecast" name="AI Forecast" stroke="#db2777" strokeWidth={3} />
                                </ComposedChart>
                            </ResponsiveContainer>
                        ...
                        <div className="flex gap-4 mt-2">
                            <div className="flex-1 bg-slate-50 rounded-lg p-3 text-center border border-slate-100">
                                <span className="text-xs text-slate-500 block font-bold uppercase">예측 정확도</span>
                                <span className="text-xl font-bold text-blue-600">96.4%</span>
                            </div>
                            <div className="flex-1 bg-slate-50 rounded-lg p-3 text-center border border-slate-100">
                                <span className="text-xs text-slate-500 block font-bold uppercase">성장률 (MoM)</span>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-500">
                                                Curr: {item.current} / Opt: {item.optimal}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                        ...
                        <div className="mt-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-indigo-800 mb-1">AI Insight</h4>
                                <p className="text-xs text-indigo-700 leading-relaxed">
                                    "Electronic Module" 재고가 최적 수준 대비 50% 초과 상태입니다. 다음 발주 수량을 500개 감소시켜 재고 비용을 약 $8,000 절감할 수 있습니다.
                                </p>
                            </div>
                        </div>
                    </div>
