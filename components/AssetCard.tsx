...
const AssetCard: React.FC<AssetCardProps> = ({ asset }) => {
  ...
  const getAiCategoryLabel = (cat: AICategory) => {
    switch (cat) {
      case AICategory.SUPPLY_CHAIN: return { label: 'SCM & Demand', color: 'bg-purple-100 text-purple-700 border-purple-200' };
      case AICategory.DIGITAL_TWIN: return { label: 'Digital Twin', color: 'bg-indigo-100 text-indigo-700 border-indigo-200' };
      case AICategory.PREDICTIVE_MAINT: return { label: 'Auto & Maint.', color: 'bg-orange-100 text-orange-700 border-orange-200' };
      case AICategory.QUALITY_INSPECTION: return { label: 'Quality & QA', color: 'bg-rose-100 text-rose-700 border-rose-200' };
      case AICategory.ENERGY_OPTIMIZATION: return { label: 'Energy & ESG', color: 'bg-teal-100 text-teal-700 border-teal-200' };
